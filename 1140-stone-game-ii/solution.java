public final class Solution {
    ...
    private static int stoneGameUncached(int n, int index, int m, int[] totalStonesCache, Integer[][] memo) {
        int midIndex = index + m;
        int lastIndex = midIndex + m;
        ...
        index++;
        int minOpponentStones = stoneGame(n, index, m, totalStonesCache, memo);
        for (index++; index <= midIndex; index++) {
            int numOpponentStones = stoneGame(n, index, m, totalStonesCache, memo);
            minOpponentStones = Math.min(minOpponentStones, numOpponentStones);
        }
        ...
        return totalStones - minOpponentStones;
    }
}